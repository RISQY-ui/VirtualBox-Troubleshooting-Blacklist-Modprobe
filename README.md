...
- [Part 1: Cause & Initial Solution](#part-1-cause--initial-solution)
- [Part 2: Blacklist & Modprobe (Advanced Solution)](#part-2-blacklist--modprobe-advanced-solution)
...
| **5. Restart Required** | Boot configuration changes must be restarted to be applied |
...
During the troubleshooting process, I discovered that:

- **Initial attempts** with just `blacklist.conf` didn't work because KVM was already loaded in memory
- **The key breakthrough** was realizing I needed to use `modprobe -r` to remove the module from active memory immediately
- **The permanent solution** required both immediate action (removing from RAM) and configuration change (blacklist setup)
- **Verification** through reboot confirmed the solution works long-term

This two-pronged approach (immediate + permanent) became the most effective strategy.
